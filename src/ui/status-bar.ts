...
export class StatusBarUI {
  ...
  private _update(): void {
    ...
    this._item.text = parts.length > 0 ? `${text} ${parts.join(' | ')}` : `${text} 就绪`;
    this._item.color = failed > 0 ? new vscode.ThemeColor('statusBarItem.warningForeground') : undefined;
    this._item.backgroundColor = failed > 0 ? new vscode.ThemeColor('statusBarItem.warningBackground') : undefined;
    this._item.show();
  }
